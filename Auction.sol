...
contract Auction { 
    ...
    event CreateAuction(address creator, uint256 startedPrice, uint32 auctionId, uint32 nftId);
    ...
    mapping(bytes32 => bool) imageHashes;   
    ...
    struct NFT {
        bool active;
        bytes32 PartOfImageHash;
        bytes32 PartOfImageHashAndName;
        address owner;
    }
    ...
    function isOriginal(string memory _imageHash) internal view returns (bool) {
        ...
        return imageHashes[bytesImageHash]; 
    }

    function getAuctionTime(uint32 _auctionId) external view returns (uint256, uint256, uint256, uint256){
        AuctionInfo memory _auction = auctions[_auctionId];
        return (_auction.startedTime, _auction.endTime, _auction.endTime -  _auction.startedTime, _auction.endTime - block.timestamp);
    }
    ...
    function getAuctionHighestBid(uint32 _auctionId) public view returns (uint256){
        return highestBid[_auctionId];
    }

    function getAuctionHighestBidder(uint32 _auctionId) public view returns (address){
    ...
    function createNFT(string memory _imageHash, string memory _name) external noReentrant(msg.sender) {  // assembly 74194 ---- string 119657 gas
        require(bytes(_name).length > 3, "Invalid NFT name");
        ...
        require(!imageHashes[imageHashPart1], "Image hash is not unique. You are trying to upload an existed image");

        bytes32 imageHashPart2;

        string memory concat = string.concat(_imageHash, _name); 
        
        assembly {
            // let hash := add(add(_imageHash, 46), 18)
            // let name := add(_name, 32)
            // let tmp := add(name, hash)
        
            imageHashPart2 := mload(add(add(concat, 46), 18))
        }
    
        nftCounter.increment();
        nfts[ uint32(nftCounter.current()) ] = NFT(false, imageHashPart1, imageHashPart2, msg.sender);
        imageHashes[imageHashPart1] = true;
       
        emit CreateNFT(msg.sender, _imageHash, _name);
    }


    function createAuction(uint256 _price, uint32 _nftId, uint16 _auctionTime) external noReentrant(msg.sender) {

        require(_price > 0, "Minimal price should be greater than 0"); 

        address creator = msg.sender;
        require(creator == ownerOf(_nftId), "You are not the owner of this NFT");    
...
// transaction cost	61972 gas  --  57853 -- 55634 
    function placeBid(uint32 _nftId, uint32 _auctionId) external payable{
        
        require(!isEnded(_auctionId), "Auction is already finished or doesn't exists");

        address sender = msg.sender;
     //   require(sender != auctions[_auctionId].nftOwner, "Auction creator can't make bids");

        uint256 amount = msg.value;
        require(amount >= highestBid[_auctionId] + auctions[_auctionId].minBid, "Your bid is unsufficient"); 
    ...
    function winnerOrSeller(address _sender, address _auctionOwner, uint32 _auctionId) internal view returns (bool) {
    ...
    function endAuction(uint32 _auctionId) internal noReentrant(msg.sender) {
        ...
        require(winnerOrSeller(msg.sender, _auction.nftOwner, _auctionId), "You are not the winner of this NFT");
        ...
        emit Transfer(_auction.nftOwner, highestBidder[_auctionId], _auction.nftId);
    ...
    function _burn(uint32 nftId) external {
        ...
        require(nfts[nftId].owner == msg.sender, "You are not the owner of this NFT");
        delete imageHashes[nfts[nftId].PartOfImageHash];
        delete nfts[nftId];
    ...
    //     nfts[1] = NFT(false, "test", "fgsdfgds", "djaskdl", msg.sender);
